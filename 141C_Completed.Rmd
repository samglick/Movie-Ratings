...
#Need sam's code on decision trees
...
# R-squared
rsquared <- 1 - sum((y - predicted_values)^2) / sum((y - mean(y))^2)
cat("MSE:", mse, "\n")
cat("R-squared:", rsquared, "\n")
...
# Prepare the data (assuming model_dataset is your dataset and TrueIMDBScore is your target variable)
x <- model_dataset[, setdiff(names(model_dataset), "TrueIMDBScore")] # Predictor variables
y <- model_dataset$TrueIMDBScore # Target variable
...
# Training model
rf_fit <- train(formula, data = model_dataset, method = "rf", trControl = train_control)

print(rf_fit)

# Access cross-validation results
cv_results_rf <- rf_fit$resample

# Calculate average MSE and R-squared across cross-validation folds
avg_mse <- mean(cv_results_rf$RMSE^2)
avg_r_squared <- mean(cv_results_rf$Rsquared)

cat("Average Mean Squared Error (MSE) across CV folds:", avg_mse, "\n")
cat("Average R-squared across CV folds:", avg_r_squared, "\n")

cv_predictions <- extractPrediction(rf_fit)
...
ggplot(plot_data, aes(x = Actual, y = Predicted)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  ggtitle("Predicted vs Actual TrueIMDBScore - Random Forest (CV)") +
  xlab("Actual TrueIMDBScore") +
  ylab("Predicted TrueIMDBScore")
```
